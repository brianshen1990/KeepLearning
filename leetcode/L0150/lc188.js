...
var maxProfit = function(k, prices) {
    ...
    for ( let i = 0 ; i < prices.length ; i++ ) {
        for ( let j = 1 ; j <= k ; j++ ) {   
            buy[j] = Math.max( buy[j], sell[j-1] - prices[i]  ); // if we buy[i] at j
            sell[j] = Math.max( sell[j], buy[j] + prices[i] ); // if we sell[i] at j 
            // ( magic, because if we haven't sell before, buy[i] would be < 0 )
        }
    }
