...
var minOperationsMaxProfit = function(customers, boardingCost, runningCost) {
    ...
    for ( let i = 0 ; i < customers.length ; i++ ) {
        ...
        const tempRes = sumCus * boardingCost - runningCost * (i+1);
        if ( tempRes > 0 && tempRes > ret ) {
            retIndex = i+1;
        }
        ret = Math.max( ret, tempRes );
    }
    let leftIndex = customers.length;
    while ( leftCus > 0 ) {
        leftIndex++;
        if ( leftCus >= 4 ) {
            sumCus += 4;
        ...
        const tempRes = sumCus * boardingCost - runningCost * leftIndex;
        if ( tempRes > 0 && tempRes > ret ) {
            retIndex = leftIndex;
        }
        ret = Math.max( ret, tempRes);
